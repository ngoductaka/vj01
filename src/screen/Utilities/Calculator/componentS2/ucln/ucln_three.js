...
const UclnThree = props => {
  ...
  useEffect(() => {
    ...
    setParamUcln_3(props.param_ucln_3);
  ...
  return (
    <View>
      <View style={{flexDirection: 'row', flexWrap: 'wrap', margin: 5}}>
        ...
        <TouchableOpacity
          onPress={() => {
            callback({key: 'PARAM_UCLN_3', value: param_ucln_3});
          }}>
          <View style={param_ucln_3 == '' ? styles.in : styles.out}>
            <MathText
              value={
                key === 'PARAM_UCLN_3'
                  ? '$$' +
                    asciimath2latex(
                      addStr(param_ucln_3, index, showText ? '∣' : '')
                        .replace(/R/g, 'root')
                        .replace(/S/g, 'sqrt')
                        .replace(/π/g, 'pi'),
                    ) +
